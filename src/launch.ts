...
import { recordLaunchedWorktree } from "./history.js";
...
export async function launchRepository(
  ...
  // Keep an explicit namespace synchronous. When the active GitHub login is
  // needed, let workspace preparation resolve it alongside the Git fetch.
  const branchNamespace = config.branchNamespace ?? (() => getGitHubLogin());
  ...
  try {
    ...
    workspace = await prepareWorkspace(repository, config, branchNamespace);
